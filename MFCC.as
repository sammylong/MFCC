package {
	...
	import org.filterbank.Filterbank;
	import org.filterbank.Filter;
	...
	public class MFCC {
		...
		public function MFCC(wavData:ByteArray) {
			...
			trace("hamming window: sample length " + hamming.length);
			...
			for (var i:int=0; i<1; i++) { // only 1 frame for now
				...
				for (var j:int=0; j<N; j++) {
					samples[N*i + j] *= hamming[j];  
				}
				// FFT
				// populate the real vector
				for (j=0; j<N; j++) {
					xRe[j] = samples[N*i + j]
				}
				// FFT object
